// Framer Code Component - Amenity Pills Styling
        ...
        const applyStyles = () => {
            amenityKeywords.forEach(keyword => {
                ...
                allElements.forEach(element => {
                    ...
                    if (htmlElement.classList.contains('amenity-pill-processed') ||
                        htmlElement.tagName === 'SCRIPT' ||
                        htmlElement.tagName === 'STYLE' ||
                        htmlElement.id === styleId) {
                        return
                    }
                    ...
                    if (text === keyword || innerText === keyword) {
                        ...
                        const hasOnlyText = Array.from(htmlElement.childNodes).every(node => 
                            node.nodeType === Node.TEXT_NODE || 
                            (node.nodeType === Node.ELEMENT_NODE && node.textContent?.trim() === keyword)
                        )
                        
                        if (hasOnlyText || text === keyword) {
                            htmlElement.classList.add('amenity-pill', 'amenity-pill-processed')
                            ...
                            htmlElement.style.fontWeight = fontWeight
                            htmlElement.style.lineHeight = '1.4'
                            htmlElement.style.whiteSpace = 'nowrap'
                            htmlElement.style.margin = '0 4px 4px 0'
                        }
                    }
                })
            })
